...
class Board (object):
	def __init__(self, data=numpy.zeros((4,4),dtype=int)):
		self.data = data

	def add_tile(self):
		data = self.data.reshape(-1)
		locs = numpy.nonzero(data == 0)[0]
		if len(locs):
	...
	def check_move(self):
		...
		if numpy.count_nonzero(self.data == 0):
			return True
		for row in self.data:
			if numpy.count_nonzero(numpy.ediff1d(row) == 0):
				return True
	...
	def move(self, dir):
		...
		for row in data:
			...
			made_move |= not numpy.all(vals == row)
			row[:] = vals
...
def run_ui():
	...
	board=Board()
	board.add_tile()
	labels = numpy.empty_like(board.data,dtype=object)
	for i in range(board.data.shape[0]):
		for j in range(board.data.shape[1]):
			labels[i,j] = ui.Label()
